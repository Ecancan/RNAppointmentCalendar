...
const DEFAULT_NAME = 'react-native-slide-calendar';
const DEFAULT_SHORT_NAME = 'RNSlideCalendar';
const DEFAULT_URL = 'https://github.com/Ecancan/RNAppointmentCalendar.git#readme';
const DEFAULT_GIT_URL = 'https://github.com/Ecancan/RNAppointmentCalendar.git';
const DEFAULT_AUTHOR_NAME = 'Ahmet CAN';
const DEFAULT_AUTHOR_EMAIL = 'email.ahmetcan@gmail.com';
...
const renameFiles = (
  ...
  try {
    ...
    if (jsOnly) {
      ...
      newPackageData = newPackageData.replace(/"files": \[.+\],/s, '"files": [\n    "lib"\n  ],');
    }
    ...
    if (jsOnly) {
      ...
      const appData = fs.readFileSync('example/src/App.tsx').toString();
      const newAppData = replaceDefaultShortName(appData, shortName).replace(DEFAULT_NAME, name);
      fs.writeFileSync('example/src/App.tsx', newAppData);
      ...
      // );
      // const schemeData = fs
      //   .readFileSync(`ios/${DEFAULT_SHORT_NAME}Module.xcodeproj/xcshareddata/xcschemes/${shortName}Module.xcscheme`)
      //   .toString();
...
//
      ...
      // fs.renameSync(`ios/${DEFAULT_SHORT_NAME}Module.xcodeproj`, `ios/${shortName}Module.xcodeproj`);
//
      // // Modify `project.pbxproj`
      // const projectData = fs.readFileSync(`ios/${shortName}Module.xcodeproj/project.pbxproj`).toString();
      // const newProjectData = replaceDefaultShortName(projectData, shortName).replace(DEFAULT_AUTHOR_NAME, authorName);
      // fs.writeFileSync(`ios/${shortName}Module.xcodeproj/project.pbxproj`, newProjectData);
//
      // // Rename and modify bridging header .h file
      // fs.renameSync(`ios/${DEFAULT_SHORT_NAME}Module-Bridging-Header.h`, `ios/${shortName}Module-Bridging-Header.h`);
//
      // // Rename and modify .m file
      // fs.renameSync(`ios/${DEFAULT_SHORT_NAME}Module.m`, `ios/${shortName}Module.m`);
      // const implementationData = fs.readFileSync(`ios/${shortName}Module.m`).toString();
      // const newImplementationData = replaceDefaultShortName(implementationData, shortName).replace(
      //   DEFAULT_AUTHOR_NAME,
      //   authorName
      // );
...
//
      ...
      // fs.renameSync(`ios/${DEFAULT_SHORT_NAME}Module.swift`, `ios/${shortName}Module.swift`);
      // const swiftData = fs.readFileSync(`ios/${shortName}Module.swift`).toString();
      // const newSwiftData = replaceDefaultShortName(swiftData, shortName).replace(DEFAULT_AUTHOR_NAME, authorName);
      // fs.writeFileSync(`ios/${shortName}Module.swift`, newSwiftData);

      // Generate Android package name from auther and module names
      const androidPackageAuthorName = authorName.replace(/\s/g, '').toLowerCase();
      ...
      const defaultAndroidPackageAuthorName = DEFAULT_AUTHOR_NAME.replace(/\s/g, '').toLowerCase();
      const defaultAndroidPackageModuleName = DEFAULT_NAME.replace(/-/g, '').toLowerCase();
      ...
      const manifestData = fs.readFileSync('android/src/main/AndroidManifest.xml').toString();
      const newManifestData = manifestData.replace(defaultAndroidPackageName, androidPackageName);
      ...
      const kotlinModuleData = fs
        ...
        .toString();
      const newKotlinModuleData = replaceDefaultShortName(kotlinModuleData, shortName).replace(
      ...
      const kotlinPackageData = fs
        ...
        .toString();
      const newKotlinPackageData = replaceDefaultShortName(kotlinPackageData, shortName).replace(
        defaultAndroidPackageName,
        androidPackageName
      );
      fs.writeFileSync(
      ...
      const exampleProjectData = fs.readFileSync('example/ios/example.xcodeproj/project.pbxproj').toString();
      const newExampleProjectData = replaceDefaultShortName(exampleProjectData, shortName);
      fs.writeFileSync('example/ios/example.xcodeproj/project.pbxproj', newExampleProjectData);

      // Modify `settings.gradle`
      const settingsData = fs.readFileSync('example/android/settings.gradle').toString();
      const newSettingsData = settingsData.replace(new RegExp(DEFAULT_NAME, 'g'), name);
      fs.writeFileSync('example/android/settings.gradle', newSettingsData);

      // Modify `build.gradle`
      const buildData = fs.readFileSync('example/android/app/build.gradle').toString();
      const newBuildData = buildData.replace(new RegExp(DEFAULT_NAME, 'g'), name);
      fs.writeFileSync('example/android/app/build.gradle', newBuildData);

      // Modify `MainApplication.kt`
      const mainApplicationData = fs
        .readFileSync('example/android/app/src/main/java/com/example/MainApplication.kt')
        .toString();
      const newMainApplicationData = replaceDefaultShortName(mainApplicationData, shortName).replace(
        defaultAndroidPackageName,
        androidPackageName
      );
